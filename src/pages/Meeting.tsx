...
//   const { remoteStreams, addRemoteStream } = useWebRTC(id);
//   const [timer, setTimer] = useState(0);
//   const [meetingValid, setMeetingValid] = useState<boolean>(false);
...
//         }
//         addRemoteStream(stream); // Add local stream to remote streams for testing
//       } catch (error) {
//         console.error('Error accessing media devices:', error);
//       }
//     };
...
export function Meeting() {
  ...
  const navigate = useNavigate();
  const user = useAuthStore((state) => state.user);
  const { participants, setMeetingId, addParticipant, removeParticipant } = useMeetingStore((state) => ({
    participants: state.participants,
    setMeetingId: state.setMeetingId,
